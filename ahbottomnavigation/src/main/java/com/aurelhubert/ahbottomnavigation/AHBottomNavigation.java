...
//						current ? itemActiveColor : itemInactiveColor, forceTint));
				icon.setImageDrawable(current ? items.get(i).getDrawable(context) : items.get(i).getDrawableDef(context));
	...
	private void createSmallItems(LinearLayout linearLayout) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (itemsEnabledStates[i]) {
				icon.setImageDrawable(currentItem == i  ? items.get(i).getDrawable(context) : items.get(i).getDrawableDef(context));
//				icon.setImageDrawable(AHHelper.getTintDrawable(items.get(i).getDrawable(context),
...
//							items.get(itemIndex).getColor(context));
				...
				icon.setImageDrawable(items.get(currentItem).getDrawableDef(context));
